...
// home hero banner 1600×506 — coloured so it stands out on the dark theme too. The page overlays two CTA buttons
// from 63% of the height down (app/page.tsx), so all copy must end above ~57% (y ≈ 288).
const heroBg = svg(1600, 506, `<defs>
    <linearGradient id="bg" x1="0" y1="0" x2="1" y2="0.6"><stop offset="0" stop-color="#1e3a8a"/><stop offset="0.55" stop-color="#2563eb"/><stop offset="1" stop-color="#7c3aed"/></linearGradient>
    <radialGradient id="glow1" cx="0.08" cy="0" r="0.5"><stop offset="0" stop-color="#22d3ee" stop-opacity="0.45"/><stop offset="1" stop-color="#22d3ee" stop-opacity="0"/></radialGradient>
    <radialGradient id="glow2" cx="0.83" cy="0.5" r="0.3"><stop offset="0" stop-color="#0b0620" stop-opacity="0.75"/><stop offset="1" stop-color="#0b0620" stop-opacity="0"/></radialGradient>
  </defs>
  <rect width="1600" height="506" fill="url(#bg)"/><rect width="1600" height="506" fill="url(#glow1)"/><rect width="1600" height="506" fill="url(#glow2)"/>`);
const heroText = svg(1600, 506, `
  <text x="96" y="146" font-family="${FONT}" font-weight="800" font-size="128" letter-spacing="-5" fill="#ffffff">Arm</text>
  <text x="100" y="222" font-family="${CJK}" font-weight="700" font-size="54" fill="#ffffff">免费发币，手续费 78% 归你</text>
  <text x="100" y="276" font-family="${CJK}" font-weight="400" font-size="30" fill="#dbeafe">推广分佣 · USDC 计价 · LP 永久锁定 · Built on Arc</text>`);
await save("public/brand/hero.jpg", sharp(heroBg)
  .composite([{ input: await cutout(440), left: 1110, top: 33 }, { input: heroText, left: 0, top: 0 }]), "jpg");
