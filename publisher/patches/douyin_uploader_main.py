...
class DouYinBaseUploader(BaseVideoUploader):
    ...
    async def fill_title_and_description(self, page: Page, title: str, description: str, tags: list[str] | None = None):
        ...
        # [peiwan patch] 图文页（post/image）的标题框 placeholder 和视频页不一样，多试几种；都找不到就截图 + 把页面上所有输入框的 placeholder 写进日志，方便对着改
        title_candidates = [
            ...
            '.title-input input, [class*="title"] input',
        ]
        desc_candidates = [
        ...
        async def _find(cands, what, total_ms=120000):
            waited = 0
                ...
                for sel in cands:
                    ...
                    try:
                        if await loc.count() and await loc.is_visible():
                            douyin_logger.info(_msg("🔎", f"{what}用选择器：{sel}"))
                            return loc
                    except Exception:
                        pass
                await asyncio.sleep(1)
            ...
            # 都没找到：留证据
            try:
                shot_dir = Path(__file__).resolve().parents[3] / "logs"
                shot_dir.mkdir(exist_ok=True)
                await page.screenshot(path=str(shot_dir / f"douyin-no-{what}-{int(asyncio.get_event_loop().time())}.png"))
                dump = await page.evaluate(
                    """() => Array.from(document.querySelectorAll('input,textarea,[contenteditable="true"]'))
                        .filter(e => e.getClientRects().length)
                        .map(e => ({tag:e.tagName, ph:e.getAttribute('placeholder')||e.getAttribute('data-placeholder')||'', cls:(e.className||'').toString().slice(0,80)}))"""
                )
                douyin_logger.error(_msg("🧩", f"页面可见输入框：{dump}"))
                douyin_logger.error(_msg("🌐", f"当前 URL：{page.url}"))
            except Exception:
                pass
            raise TimeoutError(f"抖音发布页没找到{what}框（页面可能改版，看 publisher/logs 截图与日志里的输入框列表）")

        title_input = await _find(title_candidates, "标题")
        await title_input.click()
        await page.keyboard.press("Control+KeyA")
        await page.keyboard.type(title[:30])

        description_editor = await _find(desc_candidates, "正文")
