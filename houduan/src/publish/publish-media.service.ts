...
export class PublishMediaService implements OnModuleInit {
  ...
  async build(content: string): Promise<VideoMedia> {
    ...
    const prompt = [
      STYLE + `同一个故事的${scenes.length}张连续画面，人物外貌和服装必须前后一致，画面中不要出现任何文字。`,
      `人物：${characters}`,
...
async function storyboard(paragraphs: string[]): Promise<{ characters: string; scenes: string[] }> {
  ...
  const system = [
    ...
    '要求：',
    '1. characters：故事里需要出镜的人物（包括"我"），每人一句：姓名或称呼、性别、准确年龄（年轻角色写明"年轻""少女感"）、长相气质、发型、固定服装。女性角色可以漂亮性感（身材、吊带裙、湿发、锁骨等），男性有生活感。',
